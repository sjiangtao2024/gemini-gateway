...
## 14. 流式响应说明（Gemini 与 g4f）

- **Gemini (gemini-webapi)**: 多数情况下无法直接透传网页端的真实流式输出，网关侧采用“先完成后分块”的 **模拟流式**（SSE 分片）。
- **g4f**: 是否为真实流式取决于具体 provider；若 provider 不支持流式，可能退化为一次性响应或内部缓冲后再输出。

> 建议：对外文档标注“流式可能为模拟”，并在日志中记录 provider 的流式能力。
